...
editUrl: "https://github.com/jayminwest/warren/blob/v0.17.0/docs/RUNBOOK-K8S.md"
---

<!-- GENERATED by `bun run sync:upstream` from jayminwest/warren@v0.17.0 (docs/RUNBOOK-K8S.md). DO NOT EDIT — change it upstream and re-sync. -->
...
- [`deploy/k8s/README.md`](https://github.com/jayminwest/warren/blob/v0.17.0/deploy/k8s/README.md) — the manifest **quick start**: `kubectl apply -k`, overlay layout, secret-creation commands, and the host-mode kubeconfig-401 workaround. This runbook links into it and does not restate it.
- [`docs/design/k8s-migration.md`](https://github.com/jayminwest/warren/blob/v0.17.0/docs/design/k8s-migration.md) and [`runtime-provider-contract.md`](/docs/concepts/runtimes/) — the architecture decisions behind everything in this runbook. Both are historical records of work that shipped in v0.10.0.
...
See [`deploy/k8s/README.md`](https://github.com/jayminwest/warren/blob/v0.17.0/deploy/k8s/README.md) for `--load-k3d`, `--load-kind`, and `--only`.
...
Full detail (secret creation, overlay differences, ingress notes) lives in [`deploy/k8s/README.md`](https://github.com/jayminwest/warren/blob/v0.17.0/deploy/k8s/README.md).
...
Two workarounds (`kubectl proxy`, or a token kubeconfig with `--insecure-skip-tls-verify`) are in [`deploy/k8s/README.md`](https://github.com/jayminwest/warren/blob/v0.17.0/deploy/k8s/README.md) under "Host-mode local dev".
...
The exact `kubectl create secret generic` commands are in [`deploy/k8s/README.md`](https://github.com/jayminwest/warren/blob/v0.17.0/deploy/k8s/README.md) ("Secrets — never commit real values").
...
There is one bearer token, and it is not scoped or versioned (see [SECURITY.md](https://github.com/jayminwest/warren/blob/v0.17.0/SECURITY.md)).
...
[ROADMAP.md](https://github.com/jayminwest/warren/blob/v0.17.0/ROADMAP.md) defers per-user identity until paid — the deployment, not the user account, is warren's unit of trust.
...
The control plane mints a token per operation and re-mints it on expiry (design: [`docs/design/forge-contract.md`](https://github.com/jayminwest/warren/blob/v0.17.0/docs/design/forge-contract.md) §4).
