...
editUrl: "https://github.com/jayminwest/warren/blob/v0.13.1/README.md"
---

<!-- GENERATED by `bun run sync:upstream` from jayminwest/warren@v0.13.1 (README.md). DO NOT EDIT — change it upstream and re-sync. -->

<p align="center">
  <img src="https://raw.githubusercontent.com/jayminwest/warren/v0.13.1/branding/logo.png" alt="warren — self-hostable cloud control plane" width="640">
</p>
...
[![License: MIT](https://img.shields.io/badge/License-MIT-yellow.svg)](https://github.com/jayminwest/warren/blob/v0.13.1/LICENSE)
...
[`.env.example`](https://github.com/jayminwest/warren/blob/v0.13.1/.env.example) documents the full knob set. To manage the same container declaratively, use the compose file instead. It pulls the same image and applies the same flags:
...
> **Image requirement (self-host, `local` runtime): burrow-cli 0.3.15 or newer.** In the default topology warren shares the container with [burrow](https://github.com/jayminwest/burrow) and talks to it over a unix socket. The published image pins `@os-eco/burrow-cli@0.3.15` (see [`Dockerfile`](https://github.com/jayminwest/warren/blob/v0.13.1/Dockerfile)). If you build your own image, install 0.3.15 or newer. Earlier releases predate the runtime contract warren depends on (agent spawn shape, resume support, event kinds) and fail at dispatch. Under `WARREN_RUNTIME=k8s` this does not apply, because the run pods carry their own toolchain image and no burrow.
...
Run it for yourself on a home server today. The [org-readiness roadmap](https://github.com/jayminwest/warren/blob/v0.13.1/ROADMAP.md) extends the same architecture to a 50-engineer organization without a fork.
...
37 scenario-based acceptance tests in [`scripts/acceptance/`](https://github.com/jayminwest/warren/blob/v0.13.1/scripts/acceptance/) cover the end-to-end path. They span manual runs, cron triggers, and K8s pod dispatch (OOM fast-fail, steer delivery). They also span the Postgres backend, per-run preview environments, restart recovery, cost tracking, cost analytics, the seeds-extensions roundtrip, and serial plan-run dispatch.

The active frontier is the org-readiness cluster: SSO, remote workers, MCP, audit, budgets, GitHub App auth. See [ROADMAP.md](https://github.com/jayminwest/warren/blob/v0.13.1/ROADMAP.md).
...
- **Three thin clients of one pipeline.** The web UI, the `warren` admin CLI, and the HTTP API all flow through the same composition path ([SPEC §4.3](https://github.com/jayminwest/warren/blob/v0.13.1/SPEC.md#43-the-composition-flow)).
...
- **Cluster (`k8s` runtime), the hosted target.** Deploy to Kubernetes. **GKE Autopilot is the reference cluster.** Each run is its own pod, there is no burrow, and admission caps shed load before the cluster thrashes. The canonical procedure is **[docs/RUNBOOK-K8S.md](/docs/self-host/kubernetes/)**. The manifest quick-start is [`deploy/k8s/README.md`](https://github.com/jayminwest/warren/blob/v0.13.1/deploy/k8s/README.md).

Continuous deployment ships in [`.github/workflows/deploy-gke.yml`](https://github.com/jayminwest/warren/blob/v0.13.1/.github/workflows/deploy-gke.yml).

A published GitHub release (cut by [`release.yml`](https://github.com/jayminwest/warren/blob/v0.13.1/.github/workflows/release.yml)) builds the three SHA-pinned images, publishes the control plane to `ghcr.io`, and rolls the GKE Autopilot deployment forward.
...
See [SPEC §11](https://github.com/jayminwest/warren/blob/v0.13.1/SPEC.md) for the full contracts.
...
A whitespace-only body removes the fragment entirely. Unknown names and unbalanced preview markers surface through `warren doctor`, so typos are loud. See [SPEC §11.L](https://github.com/jayminwest/warren/blob/v0.13.1/SPEC.md) for the full fragment contract.
...
- **`k8s` — scale-out.** Each run is its own Kubernetes pod, and there is no burrow. Built for clusters and GKE Autopilot. See [**the K8s runbook**](/docs/self-host/kubernetes/) and [`deploy/k8s/`](https://github.com/jayminwest/warren/blob/v0.13.1/deploy/k8s/README.md).
...
That is the default (`local`) topology. Warren and burrow share the container, a unix socket, and a bearer token (`BURROW_API_TOKEN` == `WARREN_BURROW_TOKEN`). See [SPEC §10.3](https://github.com/jayminwest/warren/blob/v0.13.1/SPEC.md#103-container-layout) for the full layout.
...
| `warren db migrate-to-postgres --from <sqlite> --to <pg-url>` | One-shot SQLite → Postgres porter ([R-13](https://github.com/jayminwest/warren/blob/v0.13.1/ROADMAP.md)) |
...
`bun run gen:docs` writes the full route list to [`docs/http-api.md`](https://github.com/jayminwest/warren/blob/v0.13.1/docs/http-api.md) from `ROUTE_TABLE`.
...
The acceptance harness in [`scripts/acceptance/`](https://github.com/jayminwest/warren/blob/v0.13.1/scripts/acceptance/) drives 37 scenarios against a live container. See [ACCEPTANCE.md](https://github.com/jayminwest/warren/blob/v0.13.1/ACCEPTANCE.md) for the runbook. See [CONTRIBUTING.md](https://github.com/jayminwest/warren/blob/v0.13.1/CONTRIBUTING.md) for branch naming, testing conventions, and PR expectations. [docs/README.md](https://github.com/jayminwest/warren/blob/v0.13.1/docs/README.md) indexes every document in the repo.
...
How the current release is scoped. Full details in [SPEC §11.D](https://github.com/jayminwest/warren/blob/v0.13.1/SPEC.md#11d-v1-security-posture-known-limitations):
...
- **SSO / per-user identity** ([R-09](https://github.com/jayminwest/warren/blob/v0.13.1/ROADMAP.md)): OIDC login replacing the shared bearer. The bearer stays as a service-account path for CI.
- **MCP support** ([R-15](https://github.com/jayminwest/warren/blob/v0.13.1/ROADMAP.md)): agents declare `mcp_servers` in their prompt frontmatter, and warren plumbs credentials into the sandbox.
- **Cross-project activity UI + stable OpenAPI** ([R-14](https://github.com/jayminwest/warren/blob/v0.13.1/ROADMAP.md)): a "what is every agent doing right now" view, plus a versioned API contract.
- **Audit log** ([R-16](https://github.com/jayminwest/warren/blob/v0.13.1/ROADMAP.md)) and **cost / concurrency guardrails** ([R-17](https://github.com/jayminwest/warren/blob/v0.13.1/ROADMAP.md)): security review and budget control once real user identity lands.
- **GitHub App auth** ([R-18](https://github.com/jayminwest/warren/blob/v0.13.1/ROADMAP.md)): installation-scoped, short-lived per-run tokens replacing the shared PAT.

Every item is additive. None changes current behavior when unconfigured. See [ROADMAP.md](https://github.com/jayminwest/warren/blob/v0.13.1/ROADMAP.md) for design sketches and sequencing.

## Security

Found a vulnerability? Please follow the disclosure process in [SECURITY.md](https://github.com/jayminwest/warren/blob/v0.13.1/SECURITY.md).
...
MIT. See [LICENSE](https://github.com/jayminwest/warren/blob/v0.13.1/LICENSE).
