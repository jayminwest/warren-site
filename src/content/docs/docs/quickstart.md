...
editUrl: "https://github.com/jayminwest/warren/blob/v0.16.1/README.md"
---

<!-- GENERATED by `bun run sync:upstream` from jayminwest/warren@v0.16.1 (README.md). DO NOT EDIT — change it upstream and re-sync. -->

<p align="center">
  <img src="https://raw.githubusercontent.com/jayminwest/warren/v0.16.1/branding/logo.png" alt="warren — self-hostable cloud control plane" width="640">
</p>
...
[![License: MIT](https://img.shields.io/badge/License-MIT-yellow.svg)](https://github.com/jayminwest/warren/blob/v0.16.1/LICENSE)
...
`:latest` tracks `main`. Pin a release tag such as `:v0.13.1` for a reproducible deploy. [CHANGELOG.md](https://github.com/jayminwest/warren/blob/v0.16.1/CHANGELOG.md) records the release history.

The four security flags relax the outer container so the sandbox runtime can nest its own user namespaces (see [docs/design/runtime-and-supervisor.md](https://github.com/jayminwest/warren/blob/v0.16.1/docs/design/runtime-and-supervisor.md)). Remove any one of them and sandbox provisioning fails.

The quickstart exports the four required variables. `WARREN_BURROW_TOKEN` must equal `BURROW_API_TOKEN`, because they are the two ends of one channel. [`.env.example`](https://github.com/jayminwest/warren/blob/v0.16.1/.env.example) documents the full knob set. To manage the same container declaratively, use the compose file instead. It pulls the same image and applies the same flags:
...
> **Image requirement (self-host, `local` runtime): burrow-cli 0.3.15 or newer.** In the default topology warren shares the container with [burrow](https://github.com/jayminwest/burrow) and talks to it over a unix socket. The published image pins `@os-eco/burrow-cli@0.3.15` (see [`Dockerfile`](https://github.com/jayminwest/warren/blob/v0.16.1/Dockerfile)). If you build your own image, install 0.3.15 or newer. Earlier releases predate the runtime contract warren depends on (agent spawn shape, resume support, event kinds) and fail at dispatch. Under `WARREN_RUNTIME=k8s` this does not apply, because the run pods carry their own toolchain image and no burrow.
...
Run it for yourself on a home server today. The [org-readiness roadmap](https://github.com/jayminwest/warren/blob/v0.16.1/ROADMAP.md) extends the same architecture to a 50-engineer organization without a fork.
...
Scenario-based acceptance tests in [`scripts/acceptance/`](https://github.com/jayminwest/warren/blob/v0.16.1/scripts/acceptance/) cover the end-to-end path. They span manual runs, cron triggers, K8s pod dispatch, Postgres, previews, restart recovery, cost analytics, the seeds-extensions roundtrip, and serial plan-run dispatch.

GitHub App mode has shipped. Set `WARREN_FORGE=app` and warren mints short-lived installation tokens per operation instead of holding a static PAT. Register an App in one browser round-trip at `GET /github-app/register` (see [the K8s runbook §2.6](/docs/self-host/kubernetes/)). The active frontier is the rest of the agent-runtime adapter seam (the registry shipped in 0.16.0) and the issue-tracker contract cut. See [ROADMAP.md](https://github.com/jayminwest/warren/blob/v0.16.1/ROADMAP.md).
...
- **Three thin clients of one pipeline.** The web UI, the `warren` admin CLI, and the HTTP API all flow through the same composition path ([docs/design/agent-composition.md](https://github.com/jayminwest/warren/blob/v0.16.1/docs/design/agent-composition.md)).
...
- **Cluster (`k8s` runtime), the hosted target.** Deploy to Kubernetes. **GKE Autopilot is the reference cluster.** Each run is its own pod, there is no burrow, and admission caps shed load before the cluster thrashes. The canonical procedure is **[docs/RUNBOOK-K8S.md](/docs/self-host/kubernetes/)**. The manifest quick-start is [`deploy/k8s/README.md`](https://github.com/jayminwest/warren/blob/v0.16.1/deploy/k8s/README.md).

Continuous deployment ships in [`.github/workflows/deploy-gke.yml`](https://github.com/jayminwest/warren/blob/v0.16.1/.github/workflows/deploy-gke.yml).

A published GitHub release (cut by [`release.yml`](https://github.com/jayminwest/warren/blob/v0.16.1/.github/workflows/release.yml)) builds the three SHA-pinned images, publishes the control plane to `ghcr.io`, and rolls the GKE Autopilot deployment forward.
...
Set `WARREN_RUNTIME=k8s` on the control-plane Deployment and follow **[docs/RUNBOOK-K8S.md](/docs/self-host/kubernetes/)**. The runbook owns the image build, manifest overlays, secrets, and admission procedure. The manifest quick-start is [`deploy/k8s/README.md`](https://github.com/jayminwest/warren/blob/v0.16.1/deploy/k8s/README.md).
...
- **Per-run cost and token usage.** Warren populates the `runs.cost_usd` and `runs.tokens_*` columns for the `pi` and `claude-code` built-ins (see [docs/design/agent-composition.md](https://github.com/jayminwest/warren/blob/v0.16.1/docs/design/agent-composition.md)). The UI run-detail page surfaces them, and `GET /analytics/cost?from=&to=&projectId=` aggregates across runs (`src/db/repos/runs.ts:listForAnalytics`). A per-run `maxCostUsd` cap in `.warren/config.yaml` cancels a run at its spend ceiling (see [docs/design/warren-config.md](https://github.com/jayminwest/warren/blob/v0.16.1/docs/design/warren-config.md)).
...
- **Issue queue.** A project with a `.seeds/` directory lets agents read the queue, claim work, file follow-ups, and close finished issues. `.seeds/` also unlocks serial plan-run dispatch and past-due `extensions.scheduledFor` triggers (see [docs/design/scheduler.md](https://github.com/jayminwest/warren/blob/v0.16.1/docs/design/scheduler.md) and [docs/design/plan-run-coordinator.md](https://github.com/jayminwest/warren/blob/v0.16.1/docs/design/plan-run-coordinator.md)). Tune the plan-run coordinator with `WARREN_PLAN_RUN_TICK_MS` (default 10s), or turn it off with `WARREN_PLAN_RUN_DISABLED=1`.
...
See the topic records under [docs/design/](https://github.com/jayminwest/warren/blob/v0.16.1/docs/design/) for the full contracts.
...
A whitespace-only body removes the fragment entirely. Unknown names and unbalanced preview markers surface through `warren doctor`, so typos are loud. See [docs/design/preview-environments.md](https://github.com/jayminwest/warren/blob/v0.16.1/docs/design/preview-environments.md) for the full fragment contract.
...
**TLS through Caddy with a wildcard cert.** TLS stays on the operator's edge (see [SECURITY.md](https://github.com/jayminwest/warren/blob/v0.16.1/SECURITY.md)). Use Caddy's DNS-01 challenge to issue `*.preview.warren.example.com`, because HTTP-01 cannot issue wildcards. Minimal Caddyfile snippet:
...
**Lifecycle knobs.** [`.env.example`](https://github.com/jayminwest/warren/blob/v0.16.1/.env.example) documents the idle-TTL, lifetime, live-count, and port-range knobs with their defaults.
...
Warren does not route cross-host preview traffic: the proxy returns **501** for off-host runs (`runs.worker_id` other than the local worker). The `k8s` runtime provider superseded the multi-worker model that once scoped this work (old R-12) — see [ROADMAP.md](https://github.com/jayminwest/warren/blob/v0.16.1/ROADMAP.md). See [docs/design/preview-environments.md](https://github.com/jayminwest/warren/blob/v0.16.1/docs/design/preview-environments.md) for the full design.
...
- **`k8s` — scale-out.** Each run is its own Kubernetes pod, and there is no burrow. Built for clusters and GKE Autopilot. See [**the K8s runbook**](/docs/self-host/kubernetes/) and [`deploy/k8s/`](https://github.com/jayminwest/warren/blob/v0.16.1/deploy/k8s/README.md).
...
That is the default (`local`) topology. Warren and burrow share the container, a unix socket, and a bearer token (`BURROW_API_TOKEN` == `WARREN_BURROW_TOKEN`). See [docs/design/runtime-and-supervisor.md](https://github.com/jayminwest/warren/blob/v0.16.1/docs/design/runtime-and-supervisor.md) for the full layout.
...
The route list comes from `ROUTE_TABLE`. `bun run gen:docs` writes [`docs/http-api.md`](https://github.com/jayminwest/warren/blob/v0.16.1/docs/http-api.md), and `bun run gen:openapi` writes [`docs/openapi.yaml`](https://github.com/jayminwest/warren/blob/v0.16.1/docs/openapi.yaml).
...
[CONTRIBUTING.md](https://github.com/jayminwest/warren/blob/v0.16.1/CONTRIBUTING.md) lists the full build-and-test command set.
...
The acceptance harness in [`scripts/acceptance/`](https://github.com/jayminwest/warren/blob/v0.16.1/scripts/acceptance/) drives end-to-end scenarios against a live container. See [ACCEPTANCE.md](https://github.com/jayminwest/warren/blob/v0.16.1/ACCEPTANCE.md) for the runbook. See [CONTRIBUTING.md](https://github.com/jayminwest/warren/blob/v0.16.1/CONTRIBUTING.md) for branch naming, testing conventions, and PR expectations. [docs/README.md](https://github.com/jayminwest/warren/blob/v0.16.1/docs/README.md) indexes every document in the repo.
...
How the current release is scoped. Full details in [SECURITY.md](https://github.com/jayminwest/warren/blob/v0.16.1/SECURITY.md#v1-security-posture-known-limitations):

- **Single bearer token.** Rotation, expiry, and scopes are not supported. Rotate by editing `.env` (or the cluster secret) and bouncing the container. Per-user identity is on the roadmap ([ROADMAP.md](https://github.com/jayminwest/warren/blob/v0.16.1/ROADMAP.md)).
...
Warren extends from "one team, one box" to a 50-engineer org on its own infra. [ROADMAP.md](https://github.com/jayminwest/warren/blob/v0.16.1/ROADMAP.md) owns the sequencing: what is in flight, what is next, and what stays out of core.

## Security

Found a vulnerability? Please follow the disclosure process in [SECURITY.md](https://github.com/jayminwest/warren/blob/v0.16.1/SECURITY.md).
...
MIT. See [LICENSE](https://github.com/jayminwest/warren/blob/v0.16.1/LICENSE).
