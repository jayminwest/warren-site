...
editUrl: "https://github.com/jayminwest/warren/blob/v0.11.0/README.md"
---

<!-- GENERATED by `bun run sync:upstream` from jayminwest/warren@v0.11.0 (README.md). DO NOT EDIT — change it upstream and re-sync. -->

<p align="center">
  <img src="https://raw.githubusercontent.com/jayminwest/warren/v0.11.0/branding/logo.png" alt="warren — self-hostable cloud control plane" width="640">
</p>
...
[![License: MIT](https://img.shields.io/badge/License-MIT-yellow.svg)](https://github.com/jayminwest/warren/blob/v0.11.0/LICENSE)
...
- **`k8s` — scale-out.** Each run is its own Kubernetes pod (the pod boundary is the sandbox); there is no burrow. Built for clusters / GKE Autopilot. See [**the K8s runbook**](/docs/self-host/kubernetes/) and [`deploy/k8s/`](https://github.com/jayminwest/warren/blob/v0.11.0/deploy/k8s/README.md).
...
Engineering teams self-hosting their own agent infrastructure. The deployment unit is one team or one org running one warren on their own box or their own cluster. Run it for yourself on a home server today; the [org-readiness roadmap](https://github.com/jayminwest/warren/blob/v0.11.0/ROADMAP.md) extends the same architecture to a 50+ engineer organization without forcing a fork.

## Status

Stable (`0.11.0`), running on GKE in continuous use against real GitHub repos, with the Kubernetes runtime (`WARREN_RUNTIME=k8s`, pod-per-run) as the supported hosted target on GKE Autopilot. The end-to-end path is covered by 38 scenario-based acceptance tests in [`scripts/acceptance/`](https://github.com/jayminwest/warren/blob/v0.11.0/scripts/acceptance/): manual runs, cron triggers, K8s pod dispatch (OOM fast-fail, steer delivery), Postgres backend, per-run preview environments, restart recovery, cost tracking, cost analytics, seeds-extensions roundtrip, serial plan-run dispatch, plan-run + Plot composition, Plot-workbench loop. The active frontier is the org-readiness cluster: SSO, remote workers, MCP, audit, budgets, GitHub App auth. See [ROADMAP.md](https://github.com/jayminwest/warren/blob/v0.11.0/ROADMAP.md).
...
Required environment variables (see [`.env.example`](https://github.com/jayminwest/warren/blob/v0.11.0/.env.example) for the full list):
...
> **Image requirement (self-host / `local` runtime): burrow-cli ≥ 0.3.15.** In the default topology warren is co-tenanted with [burrow](https://github.com/jayminwest/burrow) inside the container and talks to it over a shared unix socket. The published image pins `@os-eco/burrow-cli@0.3.15` (see [`Dockerfile`](https://github.com/jayminwest/warren/blob/v0.11.0/Dockerfile)); if you build your own image or override the runtime, install burrow-cli **0.3.15 or newer** — earlier releases predate the runtime contract warren depends on (agent spawn shape, resume support, event kinds) and will fail at dispatch. Under `WARREN_RUNTIME=k8s` this does not apply — the run pods carry their own toolchain image and no burrow is installed.
...
There is no built-in Prometheus / OpenTelemetry exporter for the `local` runtime in V1. If you need one there, the request-id + pino combination is the seam to extend; the route table (`ROUTE_TABLE` in `src/server/handlers.ts`, documented in [`docs/http-api.md`](https://github.com/jayminwest/warren/blob/v0.11.0/docs/http-api.md)) is the stable surface to instrument against. The `k8s` runtime *does* ship pod-lifecycle Prometheus metrics on `/metrics` — see the runbook.
...
Turn it on with `WARREN_RUNTIME=k8s` on the control-plane Deployment. The full operator story — deploy, secrets + rotation, RBAC, garbage collection, admission knobs, observability, and incident playbooks — is in **[docs/RUNBOOK-K8S.md](/docs/self-host/kubernetes/)**; the manifest quick-start lives in **[deploy/k8s/README.md](https://github.com/jayminwest/warren/blob/v0.11.0/deploy/k8s/README.md)**. Some LocalProvider features degrade under `k8s` (previews are off, steering is ~5s poll rather than real-time) — the runbook's capability section spells out the gaps.
...
Library agents override built-ins by name. See [SPEC §4.2](https://github.com/jayminwest/warren/blob/v0.11.0/SPEC.md#42-the-bundle-expressed-in-canopy) for the agent-as-prompt schema.
...
Recognized fragment names: `title`, `summary`, `run`, `seeds`, `preview_url_or_placeholder`, `commits`, `files_changed`, `prompt`, `trailer`. A whitespace-only body removes the fragment entirely. Unknown names + unbalanced preview markers surface via `warren doctor` so typos are loud. See [SPEC §11.L](https://github.com/jayminwest/warren/blob/v0.11.0/SPEC.md) for the full fragment contract.
...
2. **Project side.** Ship `.warren/preview.yaml` with the preview block at the top level:
   ...
   Projects that don't opt in skip the preview sub-step entirely. See [SPEC §11.L](https://github.com/jayminwest/warren/blob/v0.11.0/SPEC.md#11l-per-run-preview-environments-2026-05-14) for the full contract.
...
See [SPEC §11.L](https://github.com/jayminwest/warren/blob/v0.11.0/SPEC.md#11l-per-run-preview-environments-2026-05-14) for the full design.
...
| `warren db migrate-to-postgres --from <sqlite> --to <pg-url>` | One-shot SQLite → Postgres porter ([R-13](https://github.com/jayminwest/warren/blob/v0.11.0/ROADMAP.md)) |
...
The acceptance harness in [`scripts/acceptance/`](https://github.com/jayminwest/warren/blob/v0.11.0/scripts/acceptance/) drives 38 scenarios against a live container. See [ACCEPTANCE.md](https://github.com/jayminwest/warren/blob/v0.11.0/ACCEPTANCE.md) for the runbook.

See [CONTRIBUTING.md](https://github.com/jayminwest/warren/blob/v0.11.0/CONTRIBUTING.md) for branch naming, testing conventions, and PR expectations.
...
How the current release is scoped. Full details in [SPEC §11.D](https://github.com/jayminwest/warren/blob/v0.11.0/SPEC.md#11d-v1-security-posture-known-limitations):
...
- **Remote sandbox workers** ([R-12](https://github.com/jayminwest/warren/blob/v0.11.0/ROADMAP.md)): one warren dispatching across many runtime workers; lifts the single-host ceiling.
- **SSO / per-user identity** ([R-09](https://github.com/jayminwest/warren/blob/v0.11.0/ROADMAP.md)): OIDC login replacing the shared bearer. The bearer stays as a service-account path for CI.
- **MCP support** ([R-15](https://github.com/jayminwest/warren/blob/v0.11.0/ROADMAP.md)): agents declare `mcp_servers` in their prompt frontmatter; warren plumbs credentials into the sandbox.
- **Cross-project activity UI + stable OpenAPI** ([R-14](https://github.com/jayminwest/warren/blob/v0.11.0/ROADMAP.md)): a "what is every agent doing right now" view, plus a versioned API contract.
- **Audit log** ([R-16](https://github.com/jayminwest/warren/blob/v0.11.0/ROADMAP.md)) and **cost / concurrency guardrails** ([R-17](https://github.com/jayminwest/warren/blob/v0.11.0/ROADMAP.md)): security review and budget control once real user identity lands.
- **GitHub App auth** ([R-18](https://github.com/jayminwest/warren/blob/v0.11.0/ROADMAP.md)): installation-scoped, short-lived per-run tokens replacing the shared PAT.

All items are additive: none change current behavior when unconfigured. See [ROADMAP.md](https://github.com/jayminwest/warren/blob/v0.11.0/ROADMAP.md) for design sketches and sequencing.

## Security

Found a vulnerability? Please follow the disclosure process in [SECURITY.md](https://github.com/jayminwest/warren/blob/v0.11.0/SECURITY.md).
...
MIT. See [LICENSE](https://github.com/jayminwest/warren/blob/v0.11.0/LICENSE).
