 ...
 *
 * This is a MARKETING site, so the marketing-adjective rule is the point, not
 * an obstacle. Words such as "seamless", "powerful", "turnkey", "blazing",
 * "unleash", and "empower" stay banned. Say what the software does instead.
 *
 * YAML frontmatter is in scope for the same reason. The headline, the eyebrow,
 * and the CTA label of every page under `content/` live there, and they are
 * the first words a reader sees. Reading only the body would exempt exactly
 * the copy this gate exists to police.
 *
 * The `files` list accepts glob patterns as well as literal paths. An entry
 * that matches nothing is skipped without an error, because this repo lists a
 * prose directory before any prose lands in it.
 *
 ...
 *
 * Three modules, split along one seam each — the file-size ratchet caps a
 * file at 500 lines and these are the cohesive pieces:
 *
 *   - `prose-files.ts`      — which files are tracked (globs -> paths)
 *   - `prose-frontmatter.ts` — YAML: the string scalars in a frontmatter block
 *   - `prose-lines.ts`      — markdown: a document -> the lines a reader reads
 *   - this file             — the STE rules, the budgets, and the report
 *
 * Usage:
...
import { proseLines } from "./prose-lines.ts";
import {
	ADJECTIVAL,
	BE,
	CONTRACTIONS,
...
export function scan(): { reports: FileReport[]; stale: string[] } {
	const { sentenceWordCap, files, budgets } = loadBudgets();
	const tracked = expandPatterns(files, REPO_ROOT);
	const reports: FileReport[] = [];

	for (const rel of tracked) {
		const report = lint(readFileSync(resolve(REPO_ROOT, rel), "utf8"), sentenceWordCap);
		report.path = rel;
		reports.push(report);
	}

	const present = new Set(tracked);
	const stale = Object.keys(budgets).filter((p) => !present.has(p));
...
function main(): void {
	...
	let failed = reportList(
		"scripts/prose-budgets.json has budgets for files that do not exist:",
		stale,
